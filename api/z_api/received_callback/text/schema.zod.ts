import { z } from 'zod';

/**
* Schema Zod para evento z_api/received_callback/text
* Gerado automaticamente pelo Webhook Mapper
*/
export const ZApiReceivedCallbackTextSchema = z.object({
  headers: z.object({
    connection: z.string(),
    "content-length": z.string(),
    host: z.string(),
    "http2-settings": z.string(),
    upgrade: z.string(),
    "content-type": z.string(),
    origin: z.string(),
    server: z.string(),
    "user-agent": z.string(),
    "z-api-token": z.string()
  }),
  body: z.object({
    isStatusReply: z.boolean(),
    chatLid: z.union([z.null(), z.string()]),
    connectedPhone: z.string(),
    waitingMessage: z.boolean(),
    isEdit: z.boolean(),
    isGroup: z.boolean(),
    isNewsletter: z.boolean(),
    instanceId: z.string(),
    messageId: z.string(),
    phone: z.string(),
    fromMe: z.boolean(),
    momment: z.number(),
    status: z.string(),
    chatName: z.string(),
    senderPhoto: z.union([z.string(), z.null()]),
    senderName: z.string(),
    photo: z.union([z.string(), z.null()]),
    broadcast: z.boolean(),
    participantPhone: z.string().optional(),
    participantLid: z.union([z.string(), z.null()]),
    forwarded: z.boolean(),
    type: z.string(),
    fromApi: z.boolean(),
    text: z.object({
      message: z.string(),
      description: z.string().optional(),
      title: z.string().optional(),
      url: z.string().optional(),
      /** TRUNCATED FIELD - Original: text */,
      thumbnailUrl: z.string().describe('TRUNCATED FIELD - Original type: text').optional()
    }),
    messageExpirationSeconds: z.number().optional(),
    referenceMessageId: z.string().optional(),
    editMessageId: z.string().optional()
  })
});

export type ZApiReceivedCallbackText = z.infer<typeof ZApiReceivedCallbackTextSchema>;
