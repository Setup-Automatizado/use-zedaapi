...
All media files received from the FUNNELCHAT through its webhook have a **30-day** expiration period. After this period, all files, whether they are audio, PDFs, images, etc., will be deleted from the storage.

:::

The FUNNELCHAT offers webhook functionality to receive text and files directly into your application, without the need to store this information on our servers. This means that your application has complete control over the data received, which is particularly important for compliance with the General Data Protection Law (LGPD).
