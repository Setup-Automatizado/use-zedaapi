...
FUNNELCHAT provides its users with a queuing system that works only for messages sent through our API. This queue plays an important role in our architecture, which is to organize and order messages until they are delivered to WhatsApp. This feature is also very useful for those situations where the cell phone connected to the FUNNELCHAT goes through some instability or has no internet connection. If this occurs, that is, if the cell phone is offline for a period, as soon as you reconnect the messages will be sent to the recipient normally.
...
Whenever you connect to FUNNELCHAT it will automatically execute the queue and fire off any messages that are pending, so be very careful! 

You can delete a queue via API or better yet through the FUNNELCHAT admin panel.

:::tip Limit queue 

FUNNELCHAT allows up to 1000 messages for a disconnected cell phone in its queue before it starts rejecting new messages to the queue.
