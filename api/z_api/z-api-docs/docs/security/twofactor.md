...
A implementação de uma etapa adicional de autenticação, conhecida como autenticação de dois fatores (2FA), no painel FUNNELCHAT é uma medida de segurança essencial para proteger suas informações e instâncias contra acessos não autorizados. Esta camada extra de proteção exige que, além de sua senha normal, você forneça um segundo fator de autenticação ao fazer login.

### **Ativando o Recurso:**
Para ativar a autenticação de dois fatores no painel da FUNNELCHAT, siga os seguintes passos:

1. Acesse o painel da FUNNELCHAT e faça login com suas credenciais existentes.
...
2. Adicione uma nova conta manualmente ou escaneie o código QR gerado pelo painel FUNNELCHAT.
3. O aplicativo autenticador vinculará sua conta FUNNELCHAT ao seu dispositivo.
4. Agora, cada vez que você fizer login no painel FUNNELCHAT, será solicitado que você forneça um código de uso único gerado pelo aplicativo autenticador.
...
A implementação da autenticação de dois fatores traz diversos benefícios para a segurança de sua conta FUNNELCHAT:
...
A autenticação de dois fatores é uma camada vital de segurança que protege suas contas e informações no painel FUNNELCHAT contra ameaças cibernéticas, tornando o acesso não autorizado muito mais difícil e garantindo a integridade de seus dados. É altamente recomendável ativar essa funcionalidade para melhorar a segurança de suas operações.