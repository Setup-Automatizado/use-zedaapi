...
Não é difícil imaginar que para comunicação entre API’s vamos precisar estabelecer um protocolo de segurança entre as partes, ou seja entre o FUNNELCHAT e sua aplicação, todas as interações com nossa API precisará ser autenticada por um ID e um token.
...
Por exemplo: https://api.funnelchat/instances/SUA_INSTANCIA/token/SEU_TOKEN/send-text
