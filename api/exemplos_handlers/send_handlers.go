...
const enterpriseLicenseMarker = "-ENT-"

func (s *server) ensureEnterpriseLicense(w http.ResponseWriter, r *http.Request) bool {
	licenseKey := os.Getenv("LICENSE_KEY")
	if strings.Contains(licenseKey, enterpriseLicenseMarker) {
		return true
	}
	...
	s.Respond(
		...
		errors.New("este recurso requer licença Enterprise; entre em contato com o suporte para adquirir acesso"),
	)
	return false
}
...
func (s *server) SendButtonsMessage() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if !s.ensureEnterpriseLicense(w, r) {
			return
		}
		txtid := r.Context().Value("userinfo").(Values).Get("Id")
		client := clientManager.GetWhatsmeowClient(txtid)
		...
		if hasMedia {
			...
			case whatsmeow.MediaDocument:
				if strings.Contains(strings.ToLower(headerMediaMime), "pdf") {
					if _, thumb, err := ProcessPDFDocument(headerMediaInfo.Data); err == nil && len(thumb) > 0 {
						headerMedia.JPEGThumbnail = thumb
					}
				}
			}
		...
		resp, err = s.sendMessageWithEchoByID(context.Background(), txtid, recipient, messageToSend, whatsmeow.SendRequestExtra{ID: msgid}, forceEcho)
		if err != nil {
			CaptureError(err, r.Context())
			s.Respond(w, r, http.StatusInternalServerError, errors.New(fmt.Sprintf("Error sending message: %v", err)))
			return
...
//
// **USE CASES:**
// - 🛍️ **Product Catalogs**: Show products with prices and descriptions
