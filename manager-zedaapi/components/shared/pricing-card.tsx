...
export interface PricingCardProps {
	/** Show monthly/annual toggle (landing page only) */
	showAnnualToggle?: boolean;
	/** CTA button content — renders inside the Button */
	ctaContent: ReactNode;
	/** CTA onClick handler (dashboard) */
	onCtaClick?: () => void;
	/** CTA as link (landing page) */
	ctaHref?: string;
	/** Disable CTA (e.g. "current plan") */
	ctaDisabled?: boolean;
	/** Loading state for CTA */
	ctaLoading?: boolean;
	/** Current user plan slug (to show "Plano atual") */
	currentPlanSlug?: string | null;
	/** Callback when instance count changes (dashboard may need this) */
	onInstanceCountChange?: (count: number) => void;
	/** Callback when tier changes (dashboard needs plan slug for checkout) */
	onTierChange?: (tierSlug: string) => void;
...
export function PricingCard({
	...
	const [instanceCount, setInstanceCount] = useState(defaultInstances);
	...
	const isCurrentPlan = currentPlanSlug === tier.slug;

	function handleSetInstanceCount(value: number) {
	...
	}, [tier.slug, onTierChange]);

	const ctaButton = (() => {
		const disabled = ctaDisabled || isCurrentPlan || ctaLoading;
		const content = isCurrentPlan
			? "Plano atual"
			: ctaLoading
				? "Processando..."
				: ctaContent;
	...
	return (
		<div className="rounded-2xl border border-border bg-card p-6 shadow-lg shadow-primary/[0.03] sm:p-10">
			{/* Annual toggle (only if enabled) */}
			{showAnnualToggle && (
				<div className="mb-8 flex justify-center">
					<div
						className="inline-flex items-center gap-3 rounded-4xl border border-border bg-muted/50 p-1"
						role="radiogroup"
				...
				<div className="space-y-6">
					...
					<div className="flex items-center justify-between">
						<div className="flex items-center gap-3">
							<Button
								...
								disabled={instanceCount <= MIN_INSTANCES}
								onClick={() =>
									handleSetInstanceCount(
										Math.max(
											MIN_INSTANCES,
											instanceCount - 1,
										),
							...
							<Button
								...
								size="icon"
								className="size-8 rounded-lg"
								disabled={instanceCount >= MAX_INSTANCES}
								onClick={() =>
									handleSetInstanceCount(
										Math.min(
											MAX_INSTANCES,
					...
					<div className="relative pt-8 sm:pt-10">
						<div className="absolute -top-0 left-0 right-0 hidden sm:block">
							<div className="flex items-end justify-between px-0.5">
								{checkpoints.map((cp) => {
									const isActive = tier.slug === cp.planSlug;
									return (
										<div
											key={cp.planSlug}
					...
					<div className="space-y-2">
						...
						{showAnnualToggle && (
							<AnimatePresence>
								{annual && annualSavings > 0 && (
									...
									>
										<span className="text-xs font-medium text-primary">
											Economize R${annualSavings}/ano com
											o plano anual
										</span>
									</motion.div>
								)}
						...
						<p className="text-xs text-muted-foreground">
							Cobrança {annual ? "anual" : "mensal"}, cancele
