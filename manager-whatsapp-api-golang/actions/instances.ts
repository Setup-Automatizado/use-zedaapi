...
const createInstanceSchema = z.object({
  name: z.string().min(1, 'Instance name is required').max(100, 'Name too long'),
  sessionName: z.string().optional(),

  // Webhook URLs
  deliveryCallbackUrl: z.string().url('Invalid URL').optional().or(z.literal('')),
  receivedCallbackUrl: z.string().url('Invalid URL').optional().or(z.literal('')),
  receivedAndDeliveryCallbackUrl: z.string().url('Invalid URL').optional().or(z.literal('')),
  messageStatusCallbackUrl: z.string().url('Invalid URL').optional().or(z.literal('')),
  connectedCallbackUrl: z.string().url('Invalid URL').optional().or(z.literal('')),
  disconnectedCallbackUrl: z.string().url('Invalid URL').optional().or(z.literal('')),
  presenceChatCallbackUrl: z.string().url('Invalid URL').optional().or(z.literal('')),
...
export async function createInstance(
  ...
  try {
    ...
    const rawData = {
      ...
      receivedAndDeliveryCallbackUrl: formData.get('receivedAndDeliveryCallbackUrl') || undefined,
      messageStatusCallbackUrl: formData.get('messageStatusCallbackUrl') || undefined,
      connectedCallbackUrl: formData.get('connectedCallbackUrl') || undefined,
      disconnectedCallbackUrl: formData.get('disconnectedCallbackUrl') || undefined,
      presenceChatCallbackUrl: formData.get('presenceChatCallbackUrl') || undefined,
...
export async function disconnectInstance(
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : 'Failed to disconnect instance';
    return error(message);
  }
}
