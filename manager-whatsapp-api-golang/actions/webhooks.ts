...
export async function updateAllWebhooks(
  ...
  try {
    ...
    // Call API
    const result = await apiUpdateAllWebhooks(
      instanceId,
      instanceToken,
      {
    ...
    const message = err instanceof Error ? err.message : 'Failed to update webhooks';
    return error(message);
  }
}
...
export async function updateWebhookSettings(
  ...
  try {
    ...
    if (settings.deliveryCallbackUrl !== undefined) {
      updates.push(
        apiUpdateWebhook(instanceId, instanceToken, 'delivery', settings.deliveryCallbackUrl)
      );
    }

    if (settings.receivedCallbackUrl !== undefined) {
      updates.push(
        apiUpdateWebhook(instanceId, instanceToken, 'received', settings.receivedCallbackUrl)
      );
    }
    ...
    if (settings.connectedCallbackUrl !== undefined) {
      updates.push(
        apiUpdateWebhook(instanceId, instanceToken, 'connected', settings.connectedCallbackUrl)
      );
    }
    ...
    if (settings.notifySentByMe !== undefined) {
      await apiUpdateNotifySentByMe(instanceId, instanceToken, settings.notifySentByMe);
    }
    ...
    const message = err instanceof Error ? err.message : 'Failed to update webhook settings';
    return error(message);
  }
}
