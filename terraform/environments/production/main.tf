...
module "ecs_service_manager" {
  ...
  app_url                = var.manager_app_url != "" ? var.manager_app_url : module.cloudfront_manager[0].https_url
  whatsapp_api_url       = "http://${module.alb.alb_dns_name}"
  whatsapp_api_public_url = module.cloudfront_api.https_url
...
# --------------------------------------------------
# CloudFront - HTTPS for Manager
# --------------------------------------------------
module "cloudfront_manager" {
  source = "../../modules/cloudfront"
  count  = var.enable_manager ? 1 : 0

  environment            = local.environment
  service_name           = "manager"
  alb_dns_name           = module.alb_manager[0].alb_dns_name
  origin_protocol_policy = "http-only"
  origin_read_timeout    = 30

  tags = local.common_tags

  depends_on = [module.alb_manager]
}
