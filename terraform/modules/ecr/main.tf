...
resource "aws_ecr_repository" "api" {
  ...
  tags = merge(var.tags, {
    Name = "${var.environment}-${var.api_repository_name}"
  })
}
...
resource "aws_ecr_repository" "manager" {
  name                 = var.manager_repository_name
  image_tag_mutability = "MUTABLE"
  ...
  tags = merge(var.tags, {
    Name = "${var.environment}-${var.manager_repository_name}"
  })
}
  ...
  policy = jsonencode({
    rules = [
      ...
      },
      {
        ...
        selection = {
          tagStatus     = "tagged"
          tagPrefixList = ["v", "prod", "latest", "homolog", "staging", "develop"]
